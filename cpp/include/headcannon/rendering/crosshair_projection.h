...
#include <headcannon/math/rotation_utils.h>
#include <cmath>

namespace headcannon::rendering {
...
inline ScreenPosition ProjectCrosshair(const CrosshairProjectionParams& params) {
    ...
    float finalFwd[3];
    headcannon::math::RotateAroundAxis(postYawFwd, finalFwdAxis, cosPitch, sinPitch, finalFwd);

    // Step 1: Inverse roll around final forward axis
    float b1[3];
    float bodyIn[3] = { bx, by, bz };
    headcannon::math::RotateAroundAxis(bodyIn, finalFwd, cosRoll, -sinRoll, b1);

    // Step 2: Inverse pitch around post-yaw right axis
    float b2[3];
    headcannon::math::RotateAroundAxis(b1, finalFwdAxis, cosPitch, -sinPitch, b2);

    // Step 3: Inverse yaw around world up axis
    float worldUp[3] = { worldUpX, worldUpY, 0.0f };
    float bodyFinal[3];
    headcannon::math::RotateAroundAxis(b2, worldUp, cosYaw, -sinYaw, bodyFinal);
