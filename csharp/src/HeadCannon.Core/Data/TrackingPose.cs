...
namespace HeadCannon.Core.Data
{
    ...
    public struct TrackingPose : IEquatable<TrackingPose>
    {
        ...
        /// <summary>
        /// Clamps rotation values to the specified limits.
        /// </summary>
        public TrackingPose Clamp(RotationLimits limits)
        {
            return new TrackingPose(
                MathUtils.Clamp(Yaw, limits.YawMin, limits.YawMax),
                MathUtils.Clamp(Pitch, limits.PitchMin, limits.PitchMax),
                MathUtils.Clamp(Roll, limits.RollMin, limits.RollMax),
                TimestampTicks
            );
        }
