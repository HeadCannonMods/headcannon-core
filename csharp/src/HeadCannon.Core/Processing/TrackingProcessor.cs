...
namespace HeadCannon.Core.Processing
{
    /// <summary>
    /// Complete tracking data processing pipeline.
    /// Pipeline: raw -> offset -> deadzone -> smooth -> sensitivity -> clamp
    /// </summary>
    public sealed class TrackingProcessor : ITrackingProcessor
    {
        ...
        /// Rotation limits.
        /// </summary>
        public RotationLimits Limits { get; set; } = RotationLimits.Default;
        ...
        public TrackingPose Process(TrackingPose rawPose, bool isRemoteConnection, float deltaTime)
        {
            ...
            float outYaw = (float)_smoothedYaw * Sensitivity.Yaw;
            float outPitch = (float)_smoothedPitch * Sensitivity.Pitch;
            float outRoll = (float)_smoothedRoll * Sensitivity.Roll;

            if (Sensitivity.InvertYaw) outYaw = -outYaw;
            if (Sensitivity.InvertPitch) outPitch = -outPitch;
            if (Sensitivity.InvertRoll) outRoll = -outRoll;

            // Step 5: Apply limits
            outYaw = MathUtils.Clamp(outYaw, Limits.YawMin, Limits.YawMax);
            outPitch = MathUtils.Clamp(outPitch, Limits.PitchMin, Limits.PitchMax);
            outRoll = MathUtils.Clamp(outRoll, Limits.RollMin, Limits.RollMax);
