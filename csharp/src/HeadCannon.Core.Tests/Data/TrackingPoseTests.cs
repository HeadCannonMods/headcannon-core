...
namespace HeadCannon.Core.Tests.Data
{
    public class TrackingPoseTests
    {
        ...
        [Fact]
        public void Clamp_ClampsToLimits()
        {
            var pose = new TrackingPose(100f, -100f, 50f, 12345);
            var limits = new RotationLimits(-45f, 45f, -30f, 30f, -20f, 20f);
            TrackingPose result = pose.Clamp(limits);

            Assert.Equal(45f, result.Yaw);
            Assert.Equal(-30f, result.Pitch);
            Assert.Equal(20f, result.Roll);
        }

        [Fact]
        public void Clamp_WithinLimits_ReturnsSame()
        {
            var pose = new TrackingPose(10f, 20f, 15f, 12345);
            var limits = new RotationLimits(-45f, 45f, -45f, 45f, -45f, 45f);
            TrackingPose result = pose.Clamp(limits);

            Assert.Equal(10f, result.Yaw);
            Assert.Equal(20f, result.Pitch);
            Assert.Equal(15f, result.Roll);
        }
