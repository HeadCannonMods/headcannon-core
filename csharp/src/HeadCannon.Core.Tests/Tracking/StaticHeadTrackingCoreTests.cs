...
using System;
using Xunit;
using HeadCannon.Core.Config;
using HeadCannon.Core.Data;
using HeadCannon.Core.Tracking;

namespace HeadCannon.Core.Tests.Tracking
{
    public class StaticHeadTrackingCoreTests : IDisposable
    {
        ...
        [Fact]
        public void Processor_UsesLimitsFromConfig()
        {
            _config.Limits = new RotationLimits(-45f, 45f, -30f, 30f, -15f, 15f);
            StaticHeadTrackingCore.Initialize(_config);

            Assert.Equal(_config.Limits, StaticHeadTrackingCore.Processor.Limits);
        }
